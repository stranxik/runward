...
| hexa-adapter-pattern | applied | file:reports/eslint.json#src/lib/harness.ts; file:reports/eslint.json#src/lib/tools.ts — the boundary is REFUSED by a linter, not asserted: the committed report records both library files linted under the rule that forbids a `src/commands/**` import, so a new consumer surface reaching inward reddens CI instead of a review. consumer surfaces stay adapters: command handlers adapt the library (file:src/commands/check.ts#checkCommand), and CI/harness integrations are inert adapter samples behind the process contract (file:templates/adapters/README.md; file:docs/adr/ADR-0012-the-gate-as-a-port-with-harness-adapters.md) |
| hexa-architecture | applied | file:reports/eslint.json#src/lib/conformance.ts; file:reports/eslint.json#src/lib/mission.ts — the arrow is machine-checked (eslint.config.js, `no-restricted-imports` scoped to src/lib/**): the library may not import a command, and the committed report records these two files clean under that rule. dependencies point inward: thin CLI adapters in src/commands/ over pure logic in src/lib/ — file:src/cli.ts; file:src/lib/conformance.ts#conformance; file:src/lib/mission.ts#analyze |
...
| process-adr-and-journal | applied | adr:0001 — the mission's own decision on where its journal lives, which is the decision this rule asks for; 73 accepted product ADRs as of 2026-09-12, each dated with a trigger to revisit — file:docs/adr/ADR-0001-enforce-declared-rule-conformance-at-the-gate.md; file:docs/adr/ADR-0024-machine-surface-of-the-rule-set.md; mission bridge: file:runward/adr/ADR-0001-decision-journal-lives-in-docs-adr.md |
...
Structural decisions are locked in the product journal at `docs/adr/` — 32 accepted ADRs at this note's version ([adr/ADR-0001](adr/ADR-0001-decision-journal-lives-in-docs-adr.md) records why the journal lives there). The load-bearing ones for this note:
