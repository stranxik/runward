...
| config-secrets-boundary | applied | file:reports/secretlint.sarif — a committed secrets scan of this repository, zero findings, and its SENSITIVITY proved in the same run: the step plants three fake credentials (AWS, GitHub, npm) in a temporary directory and refuses to write the clean report unless the scanner sees all three, because a zero without a positive control does not distinguish nothing from blind. the only secret in the product's lifecycle (npm publish credential) is attached by infrastructure at the CI boundary — OIDC-minted per release, never present in the repo, the package or the CLI: file:.github/workflows/release.yml#id-token |
...
| resilience-fail-open | applied | test:reports/junit.xml::"ADR-0053: unknown --through phase throws fail-loud, never a silent all-deferred green"; the verdict fails closed — malformed rows, unknown rules, unresolvable pointers and stripped mappings are violations, never skips: file:src/lib/conformance.ts#conformance; advisory layers (coverage, proof freshness) degrade open without touching the verdict: file:src/commands/check.ts#checkCommand |
...
| checklist-pre-production-security | applied | file:reports/eslint-security.sarif#security/detect-child-process — a committed static security scan, and this rule is the one that matters most here: ADR-0054 forbids a spawn in the verdict path, and the scan records the claim holding across all of src/ with no finding. The scan's exclusions are named with their measured counts in eslint.security.config.js, never silently. file:SECURITY.md; file:.github/workflows/ci.yml — the zero-network guard and the network-isolated test leg run on every push, the security policy is published, and the product carries no secret, no key and no endpoint |
