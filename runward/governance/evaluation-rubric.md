...
**Version**: v0.18.1 · **Anchored judge**: none — there is no judge model, because there is no non-deterministic behavior to judge · **Rerun triggers**: every commit (CI), every release
...
| Standards conformance | NIST schema validation (`test/oscal-schema.js`) | the emitted OSCAL validates against the official 1.2.3 schema |
...
- **Anchored judge** → the anchors are external and versioned: the NIST OSCAL 1.2.3 schema and the pinned rule set. Neither moves silently.
