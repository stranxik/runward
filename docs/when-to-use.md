...
The default sits in a long lineage. Every time software absorbed an unstable component (I/O, frameworks, now models), the industry converged on the same answer: a stable contract in front, the unstable part behind — information hiding (Parnas, 1972), then ports and adapters (Cockburn, 2005), retold as onion (Palermo, 2008) and Clean Architecture (Martin, 2012). Any architecture of that family — onion, Clean, a functional core with an imperative shell, a modular monolith with explicit interfaces — satisfies the `hexa-*` rules as `applied`, because the rules check the substance (stable contracts, swappable edges), never the label. Choosing outside the family — a vertical-slice organization, a transaction-script CRUD — is the healthy use of `deviated`: decided, argued, written.
