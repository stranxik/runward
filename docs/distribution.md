...
| **Cursor** | Cursor Marketplace / `.cursor-plugin` | **Client, per-tool (soft)** | Cursor's end-of-turn hook does **not** hard-block; only a per-tool-call hook does. Honest: this is a reminder plus a per-tool guard, not an end-of-turn gate. |
