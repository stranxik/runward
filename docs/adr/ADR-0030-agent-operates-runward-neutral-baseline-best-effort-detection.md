...
- **Negative, accepted.** An agent in a harness with no runtime signal must be *told* which channel it is in (a one-line question), or it simply runs the baseline `check` itself. That is the correct degradation, not a failure. The best-effort detection surface ships as `runward wire` (read-only, recommendation-only, `wires:false` invariant); the wider `--json` coverage beyond `check` (status/doctor/manifest/compliance) is the remaining follow-on, additive and non-breaking.
