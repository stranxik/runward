...
**component-definition** (v1.2.3), the machine-readable layer of the evidence pack (ADR-0016). The
...
The export is a standard OSCAL 1.2.3 component-definition, so any OSCAL-aware GRC tool takes it as-is:
...
2. Pre-flight (optional, offline): validate against the NIST schema vendored in this repo —
   `npx ajv-cli validate -s test/fixtures/oscal_component_schema.v1.2.3.json -d runward/compliance/oscal-component-definition.json --spec=draft7`.
