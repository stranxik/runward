...
Holes: 159 · Equivalent: 11 · Display-only: 3 · Defence-in-depth: 0

### pointerLinks — 122 survivor(s): 117 hole · 5 equivalent
...
| 38 | ObjectLiteral | `{}` | hole | RECETTE : spec `- AC1 -> file:../outside.txt`, spec-check --json. Avant : exit 1, reason 'points outside the project tree' ; après : exit 1, reason '' ({} garde ok falsy mais failures.map(f=>f.reason… |
| 38 | StringLiteral | `ˋˋ` | hole | RECETTE : spec `- AC1 -> file:../outside.txt`, spec-check --json. Avant : reason 'file:../outside.txt — points outside the project tree' ; après : reason '' (exit 1 conservé, mesuré aussi sur file:./… |
| 41 | ConditionalExpression | `false` | hole | RECETTE : artefact = un RÉPERTOIRE code/dir, spec `- AC1 -> file:code/dir`, spec-check --json. Avant : exit 1, reason 'not a file' ; après : exit 1 mais reason 'the artifact is not present' (readFile… |
| 42 | BooleanLiteral | `true` | hole | INVERSION DE VERDICT. RECETTE : spec `- AC1 -> file:code/dir` où code/dir est un répertoire, spec-check --json. Avant : exit 1, 'gaps', reason 'not a file' ; après : exit 0, 'linked' — un critère poi… |
| 42 | ObjectLiteral | `{}` | hole | RECETTE : spec `- AC1 -> file:code/dir` (répertoire), spec-check --json. Avant : exit 1, reason 'file:code/dir — not a file' ; après : exit 1, reason ''. La remédiation disparaît. Raté parce qu'aucun… |
| 42 | StringLiteral | `ˋˋ` | hole | RECETTE : spec `- AC1 -> file:code/dir` (répertoire), spec-check --json. Avant : reason 'file:code/dir — not a file' ; après : reason '' (exit 1 conservé). Même cause de survie que les mutants 9-11 :… |
| 46 | ObjectLiteral | `{}` | hole | RECETTE : spec `- AC1 -> file:code/missing.ts` (absent), spec-check --json. Avant : exit 1, reason 'file:code/missing.ts — the artifact is not present' ; après : exit 1, reason ''. L'opérateur ne sai… |
| 46 | StringLiteral | `ˋˋ` | hole | RECETTE identique au mutant 13 : `- AC1 -> file:code/missing.ts`, avant reason 'the artifact is not present', après '' (exit 1 conservé, fonction et CLI). Survie : aucune assertion de reason sur le b… |
| 48 | ConditionalExpression | `false` | hole | INVERSION DE VERDICT — la vacuité RWD-2026-0003 exactement. RECETTE : artefact empty.ts de 0 octet (et blank.ts d'espaces), spec `- AC1 -> file:code/empty.ts`, spec-check --json. Avant : exit 1, 'the… |
| 48 | MethodExpression | `content` | hole | INVERSION DE VERDICT sur artefact BLANC. RECETTE : blank.ts contenant ' \n\t\n ' (espaces seuls), spec `- AC1 -> file:code/blank.ts`, spec-check --json. Avant : exit 1, 'the artifact is empty' ; aprè… |
| 49 | BooleanLiteral | `true` | hole | INVERSION DE VERDICT. RECETTE : `- AC1 -> file:code/empty.ts` (0 octet) et blank.ts (espaces) : avant exit 1 'the artifact is empty', après exit 0 'linked' sur les deux — l'objet de refus rend ok:tru… |
| 49 | ObjectLiteral | `{}` | hole | RECETTE : spec `- AC1 -> file:code/empty.ts` (0 octet), spec-check --json. Avant : exit 1, reason 'the artifact is empty' ; après : exit 1, reason '' (mesuré aussi sur blank.ts). Remédiation perdue ;… |
| 49 | StringLiteral | `ˋˋ` | hole | RECETTE : `- AC1 -> file:code/empty.ts`, spec-check --json. Avant : reason 'file:code/empty.ts — the artifact is empty' ; après : reason '' (exit 1 conservé ; idem blank.ts). Aucun test n'asserte ce … |
...
| 50 | EqualityOperator | `content.split("\n").length <= p.line` | hole | FAUX ROUGE de frontière. RECETTE : three.md = 'l1\nl2\nl3' (3 lignes, sans \n final), spec `- AC1 -> file:code/three.md:3`, spec-check --json. Avant : exit 0, 'linked' ; après : exit 1, reason 'the f… |
| 50 | StringLiteral | `""` | hole | INVERSION DE VERDICT : split('') compte les CARACTÈRES, pas les lignes. RECETTE : chars.md = 'abcdefghij' (1 ligne, 10 caractères), spec `- AC1 -> file:code/chars.md:5`, spec-check --json. Avant : ex… |
| 53 | ConditionalExpression | `false` | hole | INVERSION DE VERDICT. RECETTE : short.ts contenant 'const x = 1;', spec `- AC1 -> file:code/short.ts#x`, spec-check --json. Avant : exit 1, 'the `#` names no usable symbol' ; après : exit 0, 'linked'… |
| 53 | EqualityOperator | `p.symbol.trim().length <= 2` | hole | FAUX ROUGE. RECETTE : short.ts contenant 'export const ab = 2;', spec `- AC1 -> file:code/short.ts#ab`, spec-check --json. Avant : exit 0, 'linked' ; après : exit 1, 'the `#` names no usable symbol' … |
...
| 54 | ObjectLiteral | `{}` | hole | RECETTE : spec `- AC1 -> file:code/short.ts#`, spec-check --json. Avant : exit 1, reason 'the `#` names no usable symbol' ; après : exit 1, reason '' (mesuré aussi sur #x et #" x"). Le test AC3 'empt… |
| 54 | StringLiteral | `ˋˋ` | hole | RECETTE identique au mutant 26 : `- AC1 -> file:code/short.ts#`, avant reason 'file:code/short.ts# — the `#` names no usable symbol', après '' (exit 1 conservé, 3 scénarios mesurés). Aucune assertion… |
| 59 | ConditionalExpression | `false` | hole | INVERSION DE VERDICT — la famille au registre, exactement : le routage structurel SARIF sauté, `#RW001` retombe dans symbolPresent, et l'id est dans le JSON PARCE QU'il y a des findings. RECETTE : sa… |
| 61 | ConditionalExpression | `true` | hole | FAUX ROUGE généralisé sur le bras SARIF. RECETTE : sarif-green.sarif (règle RW001 connue, zéro finding), spec `- AC1 -> file:code/sarif-green.sarif#RW001`, spec-check --json. Avant : exit 0, 'linked'… |
| 61 | ConditionalExpression | `false` | hole | INVERSION DE VERDICT. RECETTE : sarif-bad.sarif ('{"$schema":...sarif...,"runs":[{' — le sniff isSarifReport passe, JSON.parse échoue), spec `- AC1 -> file:code/sarif-bad.sarif#RW001`, spec-check --j… |
| 61 | EqualityOperator | `s !== "unparseable"` | hole | DOUBLE INVERSION mesurée par le CLI : sarif-bad.sarif (corrompu) passe de exit 1 à exit 0 'linked' (faux vert : log illisible = preuve) ET sarif-green.sarif passe de exit 0 à exit 1 'looks like SARIF… |
| 61 | StringLiteral | `""` | hole | Faux vert mesuré. Recette : sarif-bad.sarif (marqueurs SARIF "version":"2.1.0"+"runs": mais JSON tronqué) + spec '- AC1 -> file:sarif-bad.sarif#RW001' ; node dist/cli.js spec-check spec.md -p . --jso… |
| 62 | BooleanLiteral | `true` | hole | Faux vert mesuré. Recette mutant 1 : livré exit 1 'not valid JSON' ; muté exit 0, linked:true — le retour ok:true est filtré comme succès par specConformance (filter !r.ok) et le critère lie sur un s… |
| 62 | ObjectLiteral | `{}` | hole | Reason effacé, mesuré. Même recette que le mutant 1 : le verdict tient (ok:undefined est falsy, exit reste 1, linked:false) mais reason devient '' dans le contrat --json (files[].criteria[].reason) e… |
| 62 | StringLiteral | `ˋˋ` | hole | Reason effacé, mesuré. Recette mutant 1 : exit 1 et linked:false conservés, mais reason '' dans --json et la sortie opérateur — 'the file looks like SARIF but is not valid JSON' est la remédiation (r… |
| 63 | ConditionalExpression | `true` | hole | Faux rouge + remédiation faussée, mesurés. Recette : sarif-clean.sarif (règle RW001 déclarée, zéro résultat) + '- AC1 -> file:sarif-clean.sarif#RW001' : livré exit 0 linked ; muté exit 1 'no rule RW0… |
| 63 | ConditionalExpression | `false` | hole | Faux vert mesuré. Recette : sarif-absent.sarif (le scan ne connaît que OTHER9) + '- AC1 -> file:sarif-absent.sarif#RW001' : livré exit 1 'no rule RW001 in the committed scan' ; muté exit 0, linked:tr… |
| 63 | EqualityOperator | `s !== "absent"` | hole | Double inversion mesurée. sarif-clean : exit 0→1 'no rule' (faux rouge sur scan vert) ; sarif-absent : exit 1→0 linked:true (faux vert sur règle jamais vérifiée) ; sarif-red : reason faussé en 'no ru… |
| 63 | StringLiteral | `""` | hole | Faux vert mesuré, identique au mutant 6 : sarif-absent.sarif exit 1→0, linked:true — s='absent' ne matche plus '' et le pointeur vers une règle inconnue du scan lie. Recette : '- AC1 -> file:sarif-ab… |
| 64 | BooleanLiteral | `true` | hole | Faux vert mesuré. sarif-absent.sarif : livré exit 1 'no rule RW001' ; muté exit 0, linked:true — le retour ok:true fait lier un critère sur une règle que le scan commité n'a jamais vérifiée. Défaut d… |
| 64 | ObjectLiteral | `{}` | hole | Reason effacé, mesuré sur sarif-absent.sarif : exit 1 et linked:false conservés (ok:undefined falsy) mais reason '' dans --json et la sortie humaine — 'no rule … in the committed scan' est la remédia… |
| 64 | StringLiteral | `ˋˋ` | hole | Reason effacé, mesuré sur sarif-absent.sarif : exit 1 conservé, reason '' dans le contrat --json et sous la ligne unlinked — la remédiation disparaît. Raté : branche SARIF jamais entrée par le filet … |
| 65 | ConditionalExpression | `true` | hole | Faux rouge mesuré. sarif-clean.sarif (règle déclarée, zéro résultat) : livré exit 0 linked ; muté exit 1 'the committed scan records open finding(s)… a red scan is not evidence' — un scan PROPRE décl… |
| 65 | ConditionalExpression | `false` | hole | LE défaut du registre, mesuré. sarif-red.sarif (résultat level 'error' pour RW001) + '- AC1 -> file:sarif-red.sarif#RW001' : livré exit 1 'a red scan is not evidence' ; muté exit 0, linked:true — un … |
| 65 | EqualityOperator | `s !== "findings"` | hole | Inversion complète mesurée : sarif-clean exit 0→1 (scan propre refusé) ET sarif-red exit 1→0 linked:true (scan rouge accepté) — le verdict SARIF est inversé dans les deux sens. Recette et commande de… |
| 65 | StringLiteral | `""` | hole | Faux vert mesuré, identique au mutant 13 : sarif-red.sarif exit 1→0, linked:true — un scan rouge commité lie. Recette : '- AC1 -> file:sarif-red.sarif#RW001', spec-check --json. Raté : le rouge-n'est… |
| 66 | BooleanLiteral | `true` | hole | Faux vert mesuré. sarif-red.sarif : livré exit 1 ; muté exit 0, linked:true — le retour même qui dit 'a red scan is not evidence' rend ok:true et le scan rouge lie. Défaut du registre au retour près.… |
| 66 | ObjectLiteral | `{}` | hole | Reason effacé, mesuré sur sarif-red.sarif : exit 1 et linked:false conservés, reason '' dans --json et la sortie opérateur — 'a red scan is not evidence' est précisément la phrase-remédiation du défa… |
| 66 | StringLiteral | `ˋˋ` | hole | Reason effacé, mesuré sur sarif-red.sarif : exit 1 conservé, reason '' — l'opérateur voit unlinked sans remédiation, dans --json comme en sortie humaine. Raté : branche SARIF jamais entrée par le fil… |
| 68 | BlockStatement | `{}` | hole | Faux vert total mesuré : eslint-red, eslint-absent ET eslint-bad passent tous de exit 1 à exit 0, linked:true — tout pointeur routé ESLint sort ok:true sans aucune vérification : rapport rouge, fichi… |
| 68 | ConditionalExpression | `false` | hole | Faux vert mesuré — le cas d'école du routage. Le rapport ESLint est re-routé vers symbolPresent ; 'src/app.js' n'est pas identifier-shaped donc content.includes() : le chemin est DANS le JSON précisé… |
| 70 | ConditionalExpression | `true` | hole | Faux rouge + remédiations faussées, mesurés. eslint-clean.json (zéro erreur) : exit 0→1 'looks like an ESLint report but is not valid JSON' — un rapport propre et parfaitement valide refusé ; eslint-… |
| 70 | ConditionalExpression | `false` | hole | Faux vert mesuré. eslint-bad.json (commence par [, porte "filePath": et "messages": mais JSON tronqué) : livré exit 1 'not valid JSON' ; muté exit 0, linked:true — e='unparseable' saute son retour, n… |
| 70 | EqualityOperator | `e !== "unparseable"` | hole | Double défaut mesuré : eslint-clean exit 0→1 'not valid JSON' (faux rouge), eslint-bad exit 1→0 linked:true (faux vert sur rapport illisible), eslint-red/eslint-absent gardent exit 1 avec le reason '… |
| 70 | StringLiteral | `""` | hole | Faux vert mesuré, identique au mutant 22 : eslint-bad.json exit 1→0, linked:true — le rapport ESLint illisible lie. Recette : '- AC1 -> file:eslint-bad.json#src/app.js', spec-check --json. Raté : auc… |
| 71 | BooleanLiteral | `true` | hole | Faux vert mesuré. eslint-bad.json : livré exit 1 ; muté exit 0, linked:true — ok:true fait lier un critère sur un rapport de lint que personne ne peut lire. Raté : aucun artefact ESLint-shaped dans l… |
| 71 | ObjectLiteral | `{}` | hole | Reason effacé, mesuré sur eslint-bad.json : exit 1 et linked:false conservés (ok:undefined falsy), reason '' dans --json et la sortie humaine — la remédiation disparaît. Raté : branche jamais entrée … |
| 71 | StringLiteral | `ˋˋ` | hole | Reason effacé, mesuré sur eslint-bad.json : exit 1 conservé, reason '' — unlinked sans remédiation dans le contrat --json comme en sortie opérateur. Raté : branche ESLint jamais entrée par le filet u… |
| 72 | ConditionalExpression | `true` | hole | Faux rouge mesuré. eslint-clean.json (src/app.js linté, zéro erreur) : exit 0→1 'no record for "src/app.js" in the committed lint report' — le fichier EST dans le rapport ; eslint-red garde exit 1 ma… |
| 72 | ConditionalExpression | `false` | hole | Faux vert mesuré. eslint-absent.json (le rapport ne mentionne que src/other.js) + '- AC1 -> file:eslint-absent.json#src/app.js' : livré exit 1 'no record… in the committed lint report' ; muté exit 0,… |
| 72 | EqualityOperator | `e !== "absent"` | hole | Double inversion mesurée : eslint-clean exit 0→1 'no record' (faux rouge sur fichier bien linté et propre), eslint-absent exit 1→0 linked:true (faux vert sur fichier jamais linté), eslint-red garde e… |
| 72 | StringLiteral | `""` | hole | Faux vert mesuré, identique au mutant 29 : eslint-absent.json exit 1→0, linked:true — un critère lie sur un rapport de lint qui ne connaît pas le fichier cité. Recette : '- AC1 -> file:eslint-absent.… |
| 73 | BooleanLiteral | `true` | hole | Faux vert du registre : un fichier ABSENT du rapport lint commité lit « linked ». Mesuré : file:reports/eslint.json#src/ghost.ts unlinked→linked, spec-check exit 1→0, verdict gaps→linked. Le rapport … |
| 73 | ObjectLiteral | `{}` | hole | Mesuré : le critère file:reports/eslint.json#src/ghost.ts reste unlinked (ok undefined est falsy) et exit 1 tenu, mais criteria[].reason du --json passe de « no record for "src/ghost.ts" in the commi… |
| 73 | StringLiteral | `ˋˋ` | hole | Mesuré : ok:false et exit 1 tenus, mais la remédiation « no record for … in the committed lint report » devient «» dans criteria[].reason (--json) et sous la ligne unlinked en sortie humaine : l'opér… |
| 74 | ConditionalExpression | `true` | hole | Faux rouge : un fichier CLEAN du rapport lint lit unlinked avec la remédiation « error-severity finding(s) » — fausse. Mesuré : file:reports/eslint.json#src/app.ts linked→unlinked, spec verte exit 0→… |
| 74 | ConditionalExpression | `false` | hole | Le faux vert du registre : un fichier avec finding severity-2 ENREGISTRÉ dans le rapport lint commité tombe hors de la chaîne if et lit « linked ». Mesuré : file:reports/eslint.json#src/dirty.ts unli… |
| 74 | EqualityOperator | `e !== "findings"` | hole | Double inversion mesurée : le fichier clean lit unlinked (« error-severity finding(s) », spec verte exit 0→1) ET le fichier à findings lit linked (exit 1→0) — faux rouge sur la preuve propre, faux ve… |
| 74 | StringLiteral | `""` | hole | e === "" n'est jamais vrai (e ∈ clean/findings/absent/unparseable) : équivalent au if(false) — le cas findings tombe hors de la chaîne. Mesuré : file:reports/eslint.json#src/dirty.ts unlinked→linked,… |
| 75 | BooleanLiteral | `true` | hole | Faux vert du registre : le critère pointant un fichier à finding severity-2 lit « linked » avec ok:true (le reason porté est ignoré car ok l'emporte). Mesuré : src/dirty.ts unlinked→linked, exit 1→0,… |
| 75 | ObjectLiteral | `{}` | hole | Mesuré : unlinked et exit 1 tenus sur src/dirty.ts, mais criteria[].reason passe de « the committed lint report records error-severity finding(s) for "src/dirty.ts" » à «» : le rouge perd sa cause et… |
| 75 | StringLiteral | `ˋˋ` | hole | Mesuré : ok:false et exit 1 tenus, remédiation « error-severity finding(s) for … » remplacée par «» dans le JSON et la sortie humaine. L'opérateur ne sait plus si le rouge vient d'un finding, d'une a… |
| 77 | BlockStatement | `{}` | hole | La branche SBOM détecte et n'adjuge plus rien : mesuré, les TROIS rouges passent verts — bare-name #lodash, composant absent #left-pad@1.3.0, SBOM invalide bom-bad.json — chacun unlinked→linked, exit… |
| 77 | ConditionalExpression | `false` | hole | Le SBOM est déclassé en fichier texte et retombe sur symbolPresent. Mesuré : le nom nu file:sbom/bom.json#lodash passe unlinked (« names no version ») → LINKED (« lodash » matche à la frontière d'ide… |
| 79 | ConditionalExpression | `true` | hole | Faux rouge : tout pointeur SBOM adjuge « components could not be read ». Mesuré : les deux pointeurs valides #lodash@4.17.21 et #pkg:npm/lodash@4.17.21 linked→unlinked, spec verte exit 0→1 ; les cas … |
| 79 | ConditionalExpression | `false` | hole | Faux vert : un fichier qui se déclare CycloneDX mais dont les components sont illisibles (bom-bad.json) tombe hors de la chaîne (b vaut « unparseable », ni ambiguous ni absent) et lit « linked ». Mes… |
| 79 | EqualityOperator | `b !== "unparseable"` | hole | Double inversion mesurée : les pointeurs valides (present) lisent « could not be read » (spec verte exit 0→1) ET le SBOM illisible lit linked (exit 1→0) ; ambiguous/absent reçoivent en plus la mauvai… |
| 79 | StringLiteral | `""` | hole | b === "" n'est jamais vrai (b ∈ present/absent/ambiguous/unparseable) : même effet mesuré que le if(false) — bom-bad.json illisible lit linked, exit 1→0. Faux vert sur l'artefact le plus dégradé, cel… |
| 80 | BooleanLiteral | `true` | hole | Faux vert : le SBOM aux components illisibles lit « linked » avec ok:true. Mesuré : AC9 unlinked→linked, exit 1→0, verdict gaps→linked. Un fichier que le parseur ne peut pas lire devient une preuve d… |
| 80 | ObjectLiteral | `{}` | hole | Mesuré : unlinked et exit 1 tenus sur bom-bad.json, mais criteria[].reason passe de « the file looks like a CycloneDX SBOM but its components could not be read » à «» : la remédiation (réparer le SBO… |
| 80 | StringLiteral | `ˋˋ` | hole | Mesuré : ok:false et exit 1 tenus, remédiation « could not be read » remplacée par «» — l'opérateur voit un rouge muet et ne distingue plus SBOM cassé / composant absent / nom sans version. Hole (rea… |
| 81 | ConditionalExpression | `true` | hole | Faux rouge : tout pointeur SBOM parseable adjuge « names no version ». Mesuré : #lodash@4.17.21 et #pkg:npm/lodash@4.17.21 (purl exact, la forme même que le message réclame) linked→unlinked, spec ver… |
| 81 | ConditionalExpression | `false` | hole | Faux vert : le nom nu file:sbom/bom.json#lodash tombe hors de la chaîne et lit « linked ». Mesuré : AC7 unlinked→linked, exit 1→0. C'est précisément le refus documenté de sbomComponentPresent (« a ba… |
| 81 | EqualityOperator | `b !== "ambiguous"` | hole | Double inversion mesurée : les deux pointeurs valides lisent « names no version » (spec verte exit 0→1), le nom nu lit linked (exit 1→0), et le composant absent reçoit la remédiation ambiguous au lie… |
| 81 | StringLiteral | `""` | hole | b === "" jamais vrai : même effet mesuré que le if(false) — le nom nu #lodash lit linked, exit 1→0. Le garde anti-« version devinée » du SBOM est neutralisé. Invisible : aucun test ne commet de SBOM … |
| 82 | BooleanLiteral | `true` | hole | Faux vert : le nom nu lit « linked » avec ok:true. Mesuré : AC7 unlinked→linked, exit 1→0, verdict gaps→linked. Un pointeur qui ne nomme aucune version prouve « épinglé » — le refus-plutôt-que-devine… |
| 82 | ObjectLiteral | `{}` | hole | Mesuré : unlinked et exit 1 tenus sur #lodash, mais criteria[].reason passe de « "lodash" names no version: cite an exact purl or name@version » à «» — c'est le reason le plus prescriptif du module (… |
| 82 | StringLiteral | `ˋˋ` | hole | Mesuré : ok:false et exit 1 tenus, mais la remédiation « cite an exact purl or name@version » devient «» : le rouge ne dit plus comment se corriger, l'opérateur ne peut pas exécuter la remédiation. H… |
| 83 | ConditionalExpression | `true` | hole | Faux rouge : tout composant PRÉSENT adjuge « no component in the committed SBOM ». Mesuré : #lodash@4.17.21 et #pkg:npm/lodash@4.17.21 linked→unlinked, spec verte exit 0→1. Le gate nie l'inventaire q… |
| 83 | ConditionalExpression | `false` | hole | Faux vert du registre : un composant ABSENT du SBOM commité (#left-pad@1.3.0) tombe hors de la chaîne et lit « linked ». Mesuré : AC8 unlinked→linked, exit 1→0. L'inventaire vouche pour ce qu'il ne c… |
| 83 | EqualityOperator | `b !== "absent"` | hole | Double inversion mesurée : les composants présents lisent « no component » (spec verte exit 0→1) et le composant absent lit linked (exit 1→0). Le verdict SBOM est exactement inversé et rien ne le voi… |
| 83 | StringLiteral | `""` | hole | b === "" jamais vrai : même effet mesuré que le if(false) — #left-pad@1.3.0 absent du SBOM lit linked, exit 1→0. Faux vert sur le cas central de la branche (le composant que la livraison ne contient … |
| 84 | BooleanLiteral | `true` | hole | RECETTE : spec `- AC2 file:sbom/bom.json#right-pad@9.9.9` sur un CycloneDX qui ne contient que left-pad@1.3.0 ; `node dist/cli.js spec-check s-sbom.md -p . --json`. Livré : exit 1, verdict gaps, « no… |
| 84 | ObjectLiteral | `{}` | hole | Mesuré : unlinked et exit 1 tenus sur #left-pad@1.3.0, mais criteria[].reason passe de « no component "left-pad@1.3.0" in the committed SBOM » à «» (undefined avalé par join) : le contrat --json livr… |
| 84 | StringLiteral | `ˋˋ` | hole | Classe reason. Même recette que M1. Mesuré : exit 1 et unlinked conservés, mais reason `""` dans le JSON et ligne de remédiation vide au CLI — l'opérateur perd « no component X in the committed SBOM … |
| 86 | BlockStatement | `{}` | hole | Le pire du lot. Branche prise mais vide : la profondeur coverage entière est silencieusement droppée. Mesuré : s-lcov.md ET s-cob.md passent exit 1 → 0, verdict gaps → linked — un fichier ABSENT du r… |
| 86 | ConditionalExpression | `false` | hole | RECETTE : lcov.info avec un record couvert, un record mesuré-mais-0-exécuté, et un fichier absent (idem cobertura.xml) ; pointeurs `file:cov/lcov.info#src/uncovered.js` etc. Branche coverage morte → … |
| 86 | LogicalOperator | `isLcovReport(content) && isCoberturaReport(…` | hole | Aucun contenu n'est à la fois lcov ET cobertura : branche morte, observable mesuré IDENTIQUE à M3 (équivalent à M3, pas au livré) — uncovered → faux vert sur les deux formats, absent → rouge avec rem… |
| 88 | ConditionalExpression | `true` | hole | Faux rouge. Tout pointeur coverage rend « no record ». RECETTE : spec toute verte (2 pointeurs vers les records couverts lcov + cobertura) : mesuré exit 0 → 1, unlinked 0 → 2 — le faux rouge manufact… |
| 88 | ConditionalExpression | `false` | hole | Un fichier ABSENT du rapport lit linked. Mesuré (lcov + cobertura) : `#src/absent.js` passe de rouge « no record … in the committed coverage report » à linked, faux vert — le rapport se porte garant … |
| 88 | EqualityOperator | `l !== "absent"` | hole | Inversion complète du premier verdict coverage, mesurée sur lcov + cobertura : covered → rouge « no record » (spec toute verte : exit 0 → 1, unlinked 2), absent → linked (faux vert), uncovered → roug… |
| 88 | StringLiteral | `""` | hole | l ∈ {covered, uncovered, absent} ne vaut jamais "" : observable mesuré identique à M7 — absent → faux vert sur lcov ET cobertura, uncovered inchangé. Même exécution manquante côté tests. |
| 89 | BooleanLiteral | `true` | hole | ok:true → le retour est filtré hors des failures. Mesuré (lcov + cobertura) : `#src/absent.js` passe de rouge à linked — faux vert du record absent, même surface que M7 par l'autre bout. Branche jama… |
| 89 | ObjectLiteral | `{}` | hole | Classe reason — et la mesure tranche le soupçon du brief : PAS de crash, PAS de faux vert. L'appelant fait `filter((r) => !r.ok)` donc `{}` (ok undefined, falsy) compte toujours unlinked, et `failure… |
| 89 | StringLiteral | `ˋˋ` | hole | Classe reason. Mesuré : verdict et exit conservés, reason `""` — même perte de remédiation que M10, par mutation directe de la chaîne. Aucun test ne matche ce texte. |
| 90 | ConditionalExpression | `true` | hole | Faux rouge : tout pointeur passant le check absent rend « 0 covered lines ». Mesuré sur spec toute verte (covered lcov + cobertura) : exit 0 → 1, unlinked 0 → 2. Un rapport de coverage vert devient i… |
| 90 | ConditionalExpression | `false` | hole | Mesuré (lcov + cobertura) : `#src/uncovered.js` (mesuré, 0 ligne exécutée) passe de rouge à linked — faux vert qui annule précisément la doctrine de l'adapter (« a file nothing exercised is not evide… |
| 90 | EqualityOperator | `l !== "uncovered"` | hole | Inversion mesurée sur lcov + cobertura : covered → rouge « 0 covered lines » (spec toute verte : exit 0 → 1, unlinked 2) ET uncovered → linked (faux vert). Les deux directions fausses, invisibles du … |
| 90 | StringLiteral | `""` | hole | l ne vaut jamais "" : observable mesuré identique à M14 — uncovered → faux vert sur les deux formats. Même exécution manquante. |
| 91 | BooleanLiteral | `true` | hole | Mesuré (lcov + cobertura) : uncovered → linked, faux vert — un fichier jamais exécuté devient une preuve de coverage. Même surface que M14 par le ok. Branche sans exécution unitaire. |
| 91 | ObjectLiteral | `{}` | hole | Classe reason, même mécanique mesurée que M10 : `{}` reste falsy-ok donc unlinked et exit 1 conservés, reason `""` via join — la remédiation « measured but nothing executed it (0 covered lines) » (= … |
| 91 | StringLiteral | `ˋˋ` | hole | Classe reason. Mesuré : verdict conservé, reason `""`. Même perte de remédiation que M17. Aucun test ne matche ce texte. |
| 97 | ConditionalExpression | `false` | hole | RECETTE : `- AC1 test:code/app.test.js::` (nom vide → le parseur fait `name \|\| undefined` donc testName undefined) et `- AC2 test:code/app.test.js::" "` (nom blanc quoted → testName " "). Livré : les… |
| 97 | ConditionalExpression | `false` | hole | Mesuré : `test:code/app.test.js::` → le second disjonct évalue `undefined.trim()` → TypeError, CLI crash, exit 1 sans JSON ni findings (le cas `::" "` reste correctement rouge via trim). Une spec val… |
| 97 | ConditionalExpression | `false` | hole | Mesuré : `test:code/app.test.js::" "` → le guard ne tire plus (seul l'undefined est couvert) → retombe sur `content.includes(" ")`, vrai sur tout fichier indenté → linked, exit 1 → 0. Faux vert du no… |
| 97 | LogicalOperator | `p.testName === undefined && p.testName.trim…` | hole | Deux effets mesurés. (1) `test:code/app.test.js::` → testName undefined → le && évalue `undefined.trim()` → TypeError « Cannot read properties of undefined (reading 'trim') » : le CLI crash, exit 1 S… |
| 97 | MethodExpression | `p.testName` | hole | `" ".length === 0` est faux : observable mesuré identique à M23 — `::" "` → linked, exit 1 → 0, faux vert. Même trou de tests (la forme quoted-blanche n'existe dans aucune fixture). |
| 98 | BooleanLiteral | `true` | hole | Mesuré : `::` vide et `::" "` lisent tous deux linked, exit 1 → 0 — même faux vert que M20 obtenu par le ok au lieu du guard : le `::` écrit est droppé en silence. Aucune fixture `::` vide dans les t… |
| 98 | ObjectLiteral | `{}` | hole | Classe reason, même mécanique mesurée que M10 : `{}` reste compté unlinked (ok undefined falsy dans le filter), join rend `""` — exit 1 conservé sur `::` vide ET `::" "`, mais la remédiation « the `:… |
| 98 | StringLiteral | `ˋˋ` | hole | Classe reason. Mesuré : verdict et exit conservés sur les deux formes, reason `""` — perte de la remédiation du `::` vide. Aucun test ne matche ce texte. |
...
### specConformance — 23 survivor(s): 16 hole · 4 equivalent · 3 display-only
...
| 136 | MethodExpression | `lines[i]` | hole | Sans trim, un item INDENTÉ ne matche plus LIST_ITEM (ancré ^-) : les sous-critères indentés sans pointeur disparaissent. RECETTE : section avec `- login works file:src/auth.ts#login` + ` - indented n… |
| 145 | ArithmeticOperator | `i - 1` | hole | Tous les numéros de ligne des findings sont faux de 2 (22 cas sur 28 diffèrent), ET le contrôle de cohérence du bundle exclut les mauvaises lignes du scan des références : la ligne située 2 au-dessus… |
...
| 147 | ConditionalExpression | `true` | hole | filter(() => true) est byte-identique au filtre supprimé : MESURÉ, les sorties de m21 sont strictement égales à celles de m20 sur les 28 cas (JSON comparés). Même recette et mêmes avant/après : `- de… |
...
| 147 | MethodExpression | `parseEvidencePointers(t)` | hole | Le filtre saute : les pointeurs `adr:` (qui ne portent JAMAIS de path, evidence.js) atteignent pointerLinks et resolve(base, undefined) jette TypeError. RECETTE : section avec `- decided adr:0005` ; … |
| 149 | StringLiteral | `""` | hole | La remédiation du gap le plus courant (critère sans aucun pointeur) devient vide. RECETTE : section avec `- no pointer here` ; spec-check --json. MESURÉ : 6 cas de la batterie diffèrent ; le JSON por… |
...
| 157 | StringLiteral | `""` | hole | Les remédiations multiples fusionnent sans séparateur. RECETTE : `- both dead file:src/missing.ts file:src/missing2.ts` ; spec-check --json. MESURÉ livré : reason `file:src/missing.ts — the artifact … |
...
| 172 | Regex | `/\b((?:AC\|FR\|NFR\|REQ\|US)-?\d)\b/g` | hole | Tout identifiant à 2 ou 3 chiffres cesse d'exister (AC12 ne matche plus du tout : pas de \\b entre 1 et 2), des DEUX côtés declared et referenced : au-delà de 9 critères, soit toute spec réelle, le c… |

### specBundleConformance — 22 survivor(s): 20 hole · 2 equivalent
...
| 207 | ArithmeticOperator | `i - 1` | hole | L'entrée déjà poussée porte line=i+1 ; la comparaison mutée cherche i-1 → jamais égal sur la même ligne → le doublon même-ligne n'est plus dédupliqué. MESURÉ (caseA : tasks.md l.3 `do AC7 and AC7 aga… |
| 207 | ArrowFunction | `() => undefined` | hole | `some()` toujours false : la déduplication meurt, un même id répété sur une même ligne est compté deux fois. RECETTE (caseA) : bundle spec.md (AC1 lié, artefact livré) + tasks.md l.3 `do AC7 and AC7 … |
| 207 | ConditionalExpression | `true` | hole | `some(true)` vaut vrai dès que `dangling` est non vide : SEUL LE PREMIER dangling du bundle est jamais rapporté. MESURÉ sur trois recettes : caseB (tasks.md `do AC7` l.3 + `later AC7 again` l.4) 2 fi… |
| 207 | ConditionalExpression | `false` | hole | Dédup morte, identique au mutant 9. MESURÉ (caseA : tasks.md l.3 `do AC7 and AC7 again`, bundle avec AC1 lié à l'artefact livré, `spec-check specs --json -p .`) : livré 1 finding {tasks.md:3,AC7} ; m… |
| 207 | ConditionalExpression | `true` | hole | Condition réduite à `d.id === m[1]` : dédup par id sur tout le bundle. MESURÉ : caseB (tasks.md `do AC7` l.3, `later AC7 again` l.4) livré 2 findings → muté 1 ; caseD (plan.md:3 et tasks.md:3, `do AC… |
...
| 207 | ConditionalExpression | `true` | hole | La dédup ignore la ligne : même fichier + même id suffit. RECETTE (caseB) : tasks.md `# Tasks`, l.3 `do AC7`, l.4 `later AC7 again`, dans un bundle dont AC1 est lié à l'artefact livré ; `spec-check s… |
...
| 207 | EqualityOperator | `d.file !== f.path` | hole | Double inversion mesurée : (a) caseA (tasks.md l.3 `do AC7 and AC7 again`) — le doublon même-fichier n'est PLUS dédupliqué, livré 1 finding → muté 2 ; (b) caseD (plan.md:3 + tasks.md:3, `do AC7`) — d… |
| 207 | EqualityOperator | `d.line !== i + 1` | hole | Double inversion mesurée : caseA (doublon même ligne, tasks.md:3 `do AC7 and AC7 again`) livré 1 finding → muté 2 (le doublon n'est plus dédupliqué : d.line!==i+1 est faux sur la même ligne) ; caseB … |
| 207 | EqualityOperator | `d.id !== m[1]` | hole | Double inversion mesurée : caseA (tasks.md:3 `do AC7 and AC7 again`) livré 1 finding → muté 2 (le vrai doublon n'est plus reconnu) ; caseC (tasks.md:3 `do AC7 then FR9`) livré [AC7,FR9] → muté [AC7] … |
| 207 | LogicalOperator | `d.file === f.path && d.line === i + 1 \|\| d.…` | hole | La dédup se fait PAR ID seul dès que l'id correspond (`\|\| d.id===m[1]`) : un même id pendant n'est plus rapporté qu'une fois dans tout le bundle. MESURÉ : caseB (même fichier, l.3 et l.4) 2 findings→… |
| 207 | LogicalOperator | `d.file === f.path \|\| d.line === i + 1` | hole | Condition devenue `(même fichier OU même ligne) ET même id` : un id pendant répété dans le MÊME fichier à des lignes différentes est dédupliqué à tort, comme un même id à même numéro de ligne dans de… |
...
| 21 | Regex | `/^#{1,6}\S.*\b(acceptance\|criteria)\b/i` | hole | Le silence parfait : par backtracking de #{1,6}, les titres ## à ###### matchent encore (le 2e # sert de \\S) ; SEUL `# Acceptance criteria` (H1) disparaît du périmètre. RECETTE : spec avec section #… |
...
| 22 | Regex | `/^(?:[-*]\s\|\d\.\s)/` | hole | Les items ordonnés à 2+ chiffres ne sont plus des critères. RECETTE : section `9. ok file:src/auth.ts#login` + `10. covers everything else` (sans pointeur) ; spec-check --json. MESURÉ livré : exit 1,… |
| 22 | Regex | `/^(?:[-*]\s\|\D+\.\s)/` | hole | Double dérive mesurée. (a) Faux vert : `1. covers something` (sans pointeur) n'est plus un critère ; livré exit 1 total 2, muté exit 0 total 1 (fixture ordered_1 ; idem ordered_10). (b) Faux rouge : … |
| 22 | Regex | `/^(?:[-*]\s\|\d+\.\S)/` | hole | `N. texte` (espace après le point, la seule graphie markdown réelle) ne matche plus. RECETTE : fixtures ordered_1 et ordered_10 ci-dessus ; spec-check --json. MESURÉ : ordered_1 livré exit 1 total 2 … |
