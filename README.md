...
New here? Follow [your first mission in 15 minutes](https://github.com/stranxik/runward/blob/main/docs/first-mission.md).
...
| `runward check --freeze` | **Seal a green gate**: hashes every resolvable evidence file into `runward/evidence-lock.json` (SHA-256, committed). A sealed file that later changes or disappears fails `check --strict` until you re-verify and re-seal — silent post-merge erosion becomes loud. Refuses to seal a red gate. **Seal what has stopped moving** — a handover, a release, a version entering operation; sealing a deliverable still under active edit makes every expected change loud, which trains you to re-seal without reading ([ADR-0021 amendment](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0021-blocking-drift-and-evidence-sealing.md)) |
| `runward check --attest` | **The verdict as a portable attestation**: wraps the `--json` verdict in an **unsigned** in-toto Statement (predicate `https://runward.dev/verdict/v1`) whose subject digest binds it to the exact mission state (mission tree ∪ cited evidence). Committing it is your gesture, and signing it stays under your own key — runward holds none ([ADR-0055](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0055-the-verdict-is-a-standards-legible-attestation.md)) |
| `runward check --sarif` | **The verdict as annotations**: emits the gate's own findings as a SARIF 2.1.0 log — each gap on the manifest row that carries it, so a reviewer reads it in the pull request instead of a CI log. Upload it with your forge's code-scanning step; runward writes the file and never calls an API. The declared non-scope travels in every rule's description, so keeping the findings while dropping the caveat has to be deliberate |
| `runward check --vsa --resource-uri <uri>` | **The verdict, in the ecosystem's own words**: a SLSA Verification Summary Attestation, so a policy engine (Kyverno, OPA) admits or refuses a deployment without learning anything about runward. It claims **no SLSA level** — `verifiedLevels` carries a custom value, because runward verifies a delivery gate and never looked at your build pipeline — and a declared horizon travels inside the level, so a prefix verdict cannot be read as a finished mission. Recipes in [`docs/interop.md`](https://github.com/stranxik/runward/blob/main/docs/interop.md) |
| `runward check --through <phase>` | **Declared-horizon gate**: certifies a *prefix* of the arc — clean through the phase you declare, every later deliverable listed as explicitly deferred (`horizon` in the JSON and the attestation) — so a green prefix can never be read as a finished mission ([ADR-0053](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0053-the-construction-gate-certifies-a-declared-horizon.md)) |
| `runward verify <attestation>` | **Offline re-check**: re-derives the mission-state digest and the verdict from the current tree — a drifted tree or a tampered predicate fails, loud. No network, no trust root, no key. A version skew (attestation produced by an older runward) is named, so verdict-logic evolution is never mistaken for forgery. Also re-hashes `bundle` manifests |
| `runward bundle <artifacts…>` | **One provenance for an assessor**: binds already-emitted artifacts (the verdict attestation, the evidence seal, an OSCAL pack, an SBOM…) into a single in-toto manifest, each referenced by raw SHA-256 — re-verifiable by `runward verify` or any cosign/in-toto tool |
| `runward spec-check <spec…>` | **Deterministic spec conformance**: every acceptance criterion in a spec (Spec Kit, OpenSpec, BMAD — format-agnostic markdown) must be *linked* to a delivered artifact that resolves, at the depth its pointer declares. Give it a **bundle** — several files, or the feature directory (`specs/<feature>/`) — and it also checks the delta's internal consistency: every criterion identifier the bundle references (`AC3` in `tasks.md`) must be declared by some file of it. Linkage and reference integrity, never a judgment that a criterion is semantically met ([ADR-0056](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0056-the-evidence-layer-widens.md)) |
...
| `runward update` | Refresh `runward/workflows/`, `rules/` and `adapters/` from the package — mission state never touched, local edits preserved unless `--force`. `--corpus <path>` vendors your org's shared rule corpus from a **local directory** (a path, never a registry coordinate): resolved from committed bytes with no fetch, the version pin recorded, drift reported — advisory, it never gates ([ADR-0057](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0057-the-shared-corpus-is-pinned-without-a-registry.md)) |
...
- **The verdict travels, and it re-checks on the repo plus the runward version that wrote it.** `check --attest` emits the verdict as an unsigned in-toto Statement bound to the exact mission state; months later `runward verify` re-derives both, offline — no service, no trust root, no key — and a version skew is named, never mistaken for forgery. `bundle` binds the delivery artifacts (attestation, seal, OSCAL, SBOM) into one provenance an assessor re-hashes with standard in-toto tooling. And the gate's cost follows what you cite, never your repo: `npm run bench` measures it on your machine — burying the same mission under 10,000 uncited files leaves `check --strict` flat.
...
- **Audit-ready evidence, in a published format.** Every CRITICAL/HIGH craft rule is ruled on against OWASP ASI (Top 10 for Agentic Applications) — mapped to its categories, or declaring in writing why it has none (a rule about hexagonal layering or cost ratios has no honest ASI category, and a forced mapping would report coverage that does not exist; ADR-0009 amendment, guarded by a test). So the conformance manifest reads as supporting evidence and exports as a machine-readable OSCAL pack. The decision → ADR → manifest → OSCAL chain is a **versioned, citable mini-spec** ([`docs/spec/runward-oscal-mapping.md`](https://github.com/stranxik/runward/blob/main/docs/spec/runward-oscal-mapping.md)) anyone can reproduce or critique — the reference implementation of gate-level agentic-delivery evidence, not a black box. Security-only by default; it maps to your regime when you need one — ISO/IEC 42001 (global), NIST AI RMF (US), or the EU AI Act technical documentation (art. 11 / Annex IV) (EU) — see [`docs/compliance/`](https://github.com/stranxik/runward/blob/main/docs/compliance/). An input to that work — never a conformity assessment, never a claim to certify. For a regulated procurement / TPRM review — what applies, what is moot because runward is **local with no data flow**, the supply-chain evidence (OIDC provenance + an attested CycloneDX **SBOM** on every release), the OpenSSF OSPS Baseline alignment, the licence framing and the honest limits — see [`docs/compliance/regulated-adoption.md`](https://github.com/stranxik/runward/blob/main/docs/compliance/regulated-adoption.md) ([ADR-0031](https://github.com/stranxik/runward/blob/main/docs/adr/ADR-0031-sovereign-engineering-evidence-for-regulated-environments.md)).
- **Never a runtime.** Runward writes nothing into `.git/`, installs nothing, and runs nothing of yours **unless you ask it to**: `check --hooks` executes the commands your own `runward/hooks.json` names, and nothing else in the tool runs a command of yours. The only other subprocess is `git`, read-only: `characterize` reads the history it inventories, and `doctor` asks `git --version`. Neither is in the verdict path. It is opt-in, it is yours, and the run prints what it ran (`Hooks · before — N ok`); a clone that does not pass the flag runs nothing at all. Otherwise it frames, gates and hands off. Your runtime and model are swappable adapters behind a port; the thing you depend on is never Runward.
...
- **One accountable operator, not a simulated team.** One human owns every gate while the agent executes the workflows — see [the operator role](https://github.com/stranxik/runward/blob/main/docs/operator-role.md).
