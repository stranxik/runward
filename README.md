...
- **Never a runtime.** Runward writes nothing into `.git/`, installs nothing, and runs nothing of yours **unless you ask it to**: `check --hooks` executes the commands your own `runward/hooks.json` names, and nothing else in the tool ever shells out. It is opt-in, it is yours, and the run prints what it ran (`Hooks · before — N ok`); a clone that does not pass the flag runs nothing at all. Otherwise it frames, gates and hands off. Your runtime and model are swappable adapters behind a port; the thing you depend on is never Runward.
