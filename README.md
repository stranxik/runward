...
| `runward check --attest` | **The verdict as a portable attestation**: wraps the `--json` verdict in an **unsigned** in-toto Statement (predicate `https://runward.dev/verdict/v1`) whose subject digest binds it to the exact mission state (mission tree ∪ cited evidence). Committing it is your gesture, and signing it stays under your own key — runward holds none ([ADR-0055](docs/adr/ADR-0055-the-verdict-is-a-standards-legible-attestation.md)) |
...
| `runward spec-check <spec…>` | **Deterministic spec conformance**: every acceptance criterion in a spec (Spec Kit, OpenSpec, BMAD — format-agnostic markdown) must be *linked* to a delivered artifact that resolves, at the depth its pointer declares. Give it a **bundle** — several files, or the feature directory (`specs/<feature>/`) — and it also checks the delta's internal consistency: every criterion identifier the bundle references (`AC3` in `tasks.md`) must be declared by some file of it. Linkage and reference integrity, never a judgment that a criterion is semantically met ([ADR-0056](docs/adr/ADR-0056-the-evidence-layer-widens.md)) |
