...
if (process.env.RW_DUMP_KEYS === "1") {
  for (const k of appeared) console.log("APPEARED\t" + JSON.stringify(k));
  for (const k of vanished) console.log("VANISHED\t" + JSON.stringify(k));
  process.exit(0);
}
const report_ = [];
if (appeared.length) {
