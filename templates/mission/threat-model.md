...
| Path / session | Private data | Untrusted content ingested | Outbound communication | Verdict |
|---|---|---|---|---|
| [path] | [yes / no] | [yes / no] | [yes / no] | [safe (2 of 3) / human supervision (3 of 3)] |

**Session rule**: at most two of the three properties at once. If all three are genuinely required on one path, the action does not run autonomously — it goes under human validation. For every three-of-three path, document which property is removed, or why supervision is imposed instead.
