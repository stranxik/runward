...
- **M1 — Resume your own mission.** Artifacts exist: framing note, ADRs, journal, code. **Opening move: run `runward status`** — it reconstructs the state from the persisted artifacts (never memory): the current gate or the iterate steady-state, the decision journal, and the *reopening watch* (each accepted ADR's reevaluation trigger — what may have aged). Read what it surfaces, judge which triggers have fired, and pick the chain back up where it stopped. `runward status` is the M1 entry because a governed mission (a `runward/` directory) already exists; `runward characterize` is for an ungoverned codebase, not this. The state of a mission is its persisted artifacts, never anyone's memory.
