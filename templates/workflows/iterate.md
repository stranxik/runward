...
- **Cross the process boundary.** Default: as late as possible. Crossing changes semantics: an atomic local access becomes a network call, strong consistency yields to eventual consistency at the global level, a transaction becomes a chain of compensable steps, causal order and idempotence under concurrency become explicit. That is the entry price of distribution — pay it only on trigger, knowingly.
